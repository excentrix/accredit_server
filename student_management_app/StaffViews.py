...
def announcements(request):
    ...
    return render(request, "staff_template/announcements2.html", context)
...
#Views for MARKS and RESULTS:

from django.shortcuts import render, redirect, get_object_or_404
from django.contrib import messages
from .models import Examination, Marks, Courses, Subjects, Students

# Examination Management Views

def examination_list(request):
    exams = Examination.objects.all()
    return render(request, 'staff_template/examination_list.html', {'exams': exams})

from student_management_app.forms import CreateExaminationForm

def create_examination(request):
    if request.method == "POST":
        form = CreateExaminationForm(request.POST)
        if form.is_valid():
            form.save()
    ...
    return render(request, 'staff_template/create_examination.html', {'form': form})
...
def edit_examination(request, exam_id):
    ...
    if request.method == "POST":
        ...
        exam.associated_courses.set(courses)
    ...
    return render(request, "staff_template/edit_examination.html", {"exam": exam, "courses": courses, "subjects": subjects})
...
def marks_list(request):
    exams = Examination.objects.all()
    return render(request, 'staff_template/marks_list.html', {'exams': exams})

def enter_marks_individual(request, exam_id):
    exam = get_object_or_404(Examination, id=exam_id)
    ...
    if request.method == 'POST':
        ...
        students = Students.objects.filter(course_id=course.id)
        subjects = exam.subjects.all()  # Assuming 'subjects' is a related field in the Examination model

        if request.method == 'POST' and 'enter_marks' in request.POST:
            for student in students:
                for subject in subjects:
                    marks = request.POST.get(f'marks_{student.id}_{subject.id}')
                    Marks.objects.create(
                        student=student,
                        exam=exam,
                        subject=subject,  # Assuming the Marks model has a 'subject' field
                        marks=marks
                    )
            return redirect('marks_list')  # Redirect after successful entry
        
        return render(request, 'staff_template/enter_marks_individual.html', {
            'exam': exam,
            'course': course,
            'students': students,
            'subjects': subjects,
    ...
    return redirect('marks_list')   # Redirect if accessed without selecting a course


    #return render(request, "staff_template/enter_marks_individual.html", {"exam": exam, "course": course, "students": students, "subjects": subjects})

def bulk_upload_marks(request, exam_id, course_id):
    exam = get_object_or_404(Examination, id=exam_id)
    course = get_object_or_404(Courses, id=course_id)

    if request.method == "POST" and request.FILES['marks_file']:
        file = request.FILES['marks_file']
        # Implement your bulk upload logic here using pandas or csv reader
        
        messages.success(request, "Marks uploaded successfully!")
        return redirect('marks_list')

    return render(request, 'staff_template/bulk_upload_marks.html', {'exam': exam, 'course': course})
